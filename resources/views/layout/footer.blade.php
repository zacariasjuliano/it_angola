<!-- FOOTER -->
<footer style="background-color:rgb(15, 0, 52)">

    <style type="text/css">
        a, a:link, a:visited, a:hover, a:active {
            color: #a8a8a8;
        }
    </style>

    <div class="row" style="padding-left: 90px;">
        <div class="col-lg-6 col-md-4">     
            
            <p style="font-size: 15pt; padding-top: 12px; line-height: 40px;">                    
                <a  href="/responsabilidade-social" style="color:rgb(255, 255, 255);">
                    Responsabilidade social
                </a><br>
                <a href="/familia#recrutamento" style="color:rgb(255, 255, 255);">
                    Recrutamento
                </a><br>
                <a href="/consultoria" style="color:rgb(255, 255, 255);">
                    Consultoria
                </a><br>
                <a href="/suporte" style="color:rgb(255, 255, 255);">
                    Suporte
                </a>
            </p>
        </div>

        <div class="col-lg-3 col-md-4">
            
            <p style="font-size: 15pt; line-height: 40px;"> 
                <a href="/blog" style="color:rgb(255, 255, 255);">
                    Blog
                </a><br>
                <a  href="/blog#newsletter" style="color:rgb(255, 255, 255);">
                    Newsletter
                </a>
            </p>
        </div>

        <div class="col-lg-3 col-md-4">
            
            <p style="font-size: 15pt; line-height: 40px;"> 
                <a href="/privacidade" style="color:rgb(255, 255, 255);">
                    Políticas
                </a><br> 
                <a href="#" style="color:rgb(255, 255, 255);">
                    Cookies
                </a>
            </p>
        </div>
    </div>

    <div class="row" style="padding-top: 0px; padding-left:90px">    
        <div class="col-lg-8">
            <!-- Social -->
            <div style="padding-left: 0px; padding-top: 30px;">          
                <a href="https://www.facebook.com/itAngola.bg" target="_blank"><i class="icon-facebook_circle text-size-50" style="color:rgb(255, 255, 255); margin-right: 10px; font-size: 40px;"></i></a> 
                <a href="https://www.instagram.com/it_angola/" target="_blank"><i class="fa fa-instagram text-size-50" style="color:rgb(255, 255, 255); margin-right: 10px; font-size: 40px;"></i></a> 
                <a href="https://www.linkedin.com/in/it-angola-549a89208/" target="_blank"><i class="icon-linked_in_circle text-size-50" style="color:rgb(255, 255, 255); margin-right: 10px; font-size: 40px;"></i></a>  
                <a href="#" target="_blank"><i class="fa fa-youtube-play text-size-50" style="color:rgb(255, 255, 255); margin-right: 10px; font-size: 40px;"></i></a>                                                                      
            </div>
        </div>
    </div> 
    
    <hr style="background-color:white; margin-left: 3%; margin-right: 9%; width: 85%">                           

    <div class="row" style="color:rgb(196, 196, 196); padding-top: -6px"> 

        <div class="col-lg-4" style="margin-left: 4%;">
            <p for="name" style="color:rgb(255, 255, 255); font-size: 15pt"> Acrescentamos Valor Ao Seu Negócio </p>
        </div>    
        <div class="col-lg-7" style="padding-right: 9%">
            <p class="text-right" style="color:rgb(255, 255, 255); font-size: 15pt"> IT-Angola - © <?php echo date("Y"); ?>, All rights reserved.</p>
        </div>
            
    </div>
    
</footer>